...
NS_OBJECT_ENSURE_REGISTERED (ThreeGppIndoorFactorySLChannelConditionModel);

TypeId
ThreeGppIndoorFactorySLChannelConditionModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactorySLChannelConditionModel")
    .SetParent<ThreeGppChannelConditionModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<ThreeGppIndoorFactorySLChannelConditionModel> ()
    .AddAttribute ("LowClutterDensity", "Clutter density (r)- percentage of surface area occupied by clutter (< 0.4). (InF-SL, InF-SH).",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&ThreeGppIndoorFactorySLChannelConditionModel::m_lowClutterDensity),
                   MakeDoubleChecker<double> (0.0, 0.39))
  ;
  return tid;
}

ThreeGppIndoorFactorySLChannelConditionModel::ThreeGppIndoorFactorySLChannelConditionModel ()
  : ThreeGppChannelConditionModel ()
{
}
...
double
ThreeGppIndoorFactorySLChannelConditionModel::ComputePlos (Ptr<const MobilityModel> a,
                                                            Ptr<const MobilityModel> b) const
{
  // compute the 2D distance between a and b
  double distance2D = Calculate2dDistance (a->GetPosition (), b->GetPosition ());
  ...
  double pLos = 0.0;
  double dClutter = 10;
  pLos = exp (-distance2D/(-dClutter / log(1-m_lowClutterDensity))); //r < 40%
...
NS_OBJECT_ENSURE_REGISTERED (ThreeGppIndoorFactoryDLChannelConditionModel);

TypeId
ThreeGppIndoorFactoryDLChannelConditionModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactoryDLChannelConditionModel")
    .SetParent<ThreeGppChannelConditionModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<ThreeGppIndoorFactoryDLChannelConditionModel> ()
    .AddAttribute ("HighClutterDensity", "Clutter density (r)- percentage of surface area occupied by clutter (>= 0.4). (InF-DL, InF-DH).",
                    DoubleValue (0.6),
                    MakeDoubleAccessor (&ThreeGppIndoorFactoryDLChannelConditionModel::m_highClutterDensity),
                    MakeDoubleChecker<double> (0.4, 1.0))
  ;
  return tid;
}

ThreeGppIndoorFactoryDLChannelConditionModel::ThreeGppIndoorFactoryDLChannelConditionModel ()
  : ThreeGppChannelConditionModel ()
{
}
...
double
ThreeGppIndoorFactoryDLChannelConditionModel::ComputePlos (Ptr<const MobilityModel> a,
                                                            Ptr<const MobilityModel> b) const
{
  // compute the 2D distance between a and b
  double distance2D = Calculate2dDistance (a->GetPosition (), b->GetPosition ());
  ...
  double pLos = 0.0;
  double dClutter = 2;
  pLos = exp (-distance2D/(-dClutter / log(1-m_highClutterDensity))); //r >= 40%
...
NS_OBJECT_ENSURE_REGISTERED (ThreeGppIndoorFactorySHChannelConditionModel);

TypeId
ThreeGppIndoorFactorySHChannelConditionModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactorySHChannelConditionModel")
    .SetParent<ThreeGppChannelConditionModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<ThreeGppIndoorFactorySHChannelConditionModel> ()
    .AddAttribute ("LowClutterDensity", "Clutter density (r)- percentage of surface area occupied by clutter (< 0.4). (InF-SL, InF-SH).",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&ThreeGppIndoorFactorySHChannelConditionModel::m_lowClutterDensity),
                   MakeDoubleChecker<double> (0.0, 0.39))
    .AddAttribute ("ClutterHeight", "Clutter height (m) < ceilling height (10 m)",
                   DoubleValue (5),
                   MakeDoubleAccessor (&ThreeGppIndoorFactorySHChannelConditionModel::m_clutterHeight),
                   MakeDoubleChecker<double> (0.0, 10.0))
  ;
  return tid;
}

ThreeGppIndoorFactorySHChannelConditionModel::ThreeGppIndoorFactorySHChannelConditionModel ()
  : ThreeGppChannelConditionModel ()
{
}
...
double
ThreeGppIndoorFactorySHChannelConditionModel::ComputePlos (Ptr<const MobilityModel> a,
                                                            Ptr<const MobilityModel> b) const
{
  // compute the 2D distance between a and b
  double distance2D = Calculate2dDistance (a->GetPosition (), b->GetPosition ());
  ...
  pLos = exp (-distance2D / ((-dClutter / log(1-m_lowClutterDensity)) * ((h_BS - h_UT) / (m_clutterHeight - h_UT)))); //r < 40%
...
NS_OBJECT_ENSURE_REGISTERED (ThreeGppIndoorFactoryDHChannelConditionModel);

TypeId
ThreeGppIndoorFactoryDHChannelConditionModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactoryDHChannelConditionModel")
    .SetParent<ThreeGppChannelConditionModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<ThreeGppIndoorFactoryDHChannelConditionModel> ()
    .AddAttribute ("HighClutterDensity", "Clutter density (r)- percentage of surface area occupied by clutter (>= 0.4). (InF-DL, InF-DH).",
                    DoubleValue (0.6),
                    MakeDoubleAccessor (&ThreeGppIndoorFactoryDHChannelConditionModel::m_highClutterDensity),
                    MakeDoubleChecker<double> (0.4, 1.0))
    .AddAttribute ("ClutterHeight", "Clutter height (m) < ceilling height (10 m)",
                    DoubleValue (5),
                    MakeDoubleAccessor (&ThreeGppIndoorFactoryDHChannelConditionModel::m_clutterHeight),
                    MakeDoubleChecker<double> (0.0, 10.0))
  ;
  return tid;
}

ThreeGppIndoorFactoryDHChannelConditionModel::ThreeGppIndoorFactoryDHChannelConditionModel ()
  : ThreeGppChannelConditionModel ()
{
}
...
double
ThreeGppIndoorFactoryDHChannelConditionModel::ComputePlos (Ptr<const MobilityModel> a,
                                                            Ptr<const MobilityModel> b) const
{
  // compute the 2D distance between a and b
  double distance2D = Calculate2dDistance (a->GetPosition (), b->GetPosition ());
  ...
  pLos = exp (-distance2D / ((-dClutter / log(1-m_highClutterDensity)) * ((h_BS - h_UT) / (m_clutterHeight - h_UT)))); //r > 40%
