    ...
    m_s5LinkMtu (3000)
{
  ...
  m_x2Ipv4AddressHelper.SetBase (m_x2BaseAddress.c_str(), "255.255.255.252");
  m_s11Ipv4AddressHelper.SetBase (m_s11BaseAddress.c_str(), "255.255.255.252");
  m_s5Ipv4AddressHelper.SetBase (m_s5BaseAddress.c_str(), "255.255.255.252");

  // we use a /8 net for all UEs
  m_uePgwAddressHelper.SetBase (m_UeBaseAddress.c_str(), "255.0.0.0");
...
NoBackhaulEpcHelper::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::NoBackhaulEpcHelper")
    ...
    .AddAttribute ("UeBaseAddress",
                   "Select UE base Address, for example, 7.0.0.0",
                   StringValue ("7.0.0.0"),
                   MakeStringAccessor (&NoBackhaulEpcHelper::m_UeBaseAddress),
                   MakeStringChecker ())
    .AddAttribute ("X2BaseAddress",
                   "Select X2 base Address",
                   StringValue("12.0.0.0"),
                   MakeStringAccessor (&NoBackhaulEpcHelper::m_x2BaseAddress),
                   MakeStringChecker ())
    .AddAttribute ("S11BaseAddress",
                   "Select S11 base Address",
                   StringValue("13.0.0.0"),
                   MakeStringAccessor (&NoBackhaulEpcHelper::m_s11BaseAddress),
                   MakeStringChecker ())
    .AddAttribute ("S5BaseAddress",
                   "Select S5 base Address",
                   StringValue("14.0.0.0"),
                   MakeStringAccessor (&NoBackhaulEpcHelper::m_s5BaseAddress),
                   MakeStringChecker ())
  ;
  return tid;
}
