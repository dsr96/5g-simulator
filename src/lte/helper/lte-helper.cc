  ...
  static TypeId
    ...
    .AddAttribute ("UseIdealConnection",
                  ...
                  "is true this will be ignored.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&LteHelper::m_useIdealConnection),
                   MakeBooleanChecker ())
    .AddAttribute ("UseInactiveState",
                  "Use 5G Inactive STATE.",
                  BooleanValue (false),
                  MakeBooleanAccessor (&LteHelper::m_useInactiveState),
                  MakeBooleanChecker ())
  ;
  return tid;
}
  ...
  if (m_useIdealRrc)
    {
      ...
      rrc->SetConnectionMode (m_useIdealConnection); // (rrc enb)
      rrc->SetInactiveState (m_useInactiveState); // (rrc enb)
    }
  else
    {
...
void
LteHelper::PowerOn (NetDeviceContainer ueDevices)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer::Iterator i = ueDevices.Begin (); i != ueDevices.End (); ++i)
    {
...
void
LteHelper::PowerOn (Ptr<NetDevice> ueDevice)
{
  NS_LOG_FUNCTION (this);
...
void
LteHelper::PowerOn (NetDeviceContainer ueDevices, Ptr<NetDevice> enbDevice)
{
  NS_LOG_FUNCTION (this);
  for (NetDeviceContainer::Iterator i = ueDevices.Begin (); i != ueDevices.End (); ++i)
    {
...
void
LteHelper::PowerOn (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  NS_LOG_FUNCTION (this);
  //enbRrc->SetCellId (enbDevice->GetObject<LteEnbNetDevice> ()->GetCellId ());
  ...
  Ptr<EpcUeNas> ueNas = ueLteDevice->GetNas ();
  ueNas->PowerOn (enbLteDevice->GetCellId (), enbLteDevice->GetDlEarfcn ());
