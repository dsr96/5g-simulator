...
UeManager::UeManager (Ptr<LteEnbRrc> rrc, uint16_t rnti, State s, uint8_t componentCarrierId)
  : m_lastAllocatedDrbid (0),
    ...
    m_disconnectEventID(),
    m_inactivity(Seconds(10)),
    m_packetDuplication (false)
{
...
void
UeManager::SetConnectionMode (bool useIdeal)
{
  NS_LOG_FUNCTION (this);
  m_useIdeal = useIdeal;
}
...
void
UeManager::SetInactiveState (bool useInactiveState)
{
  NS_LOG_FUNCTION (this);
...
void
UeManager::SetInactivityTimer (Time inactivityTime)
{
  NS_LOG_FUNCTION (this);
  m_inactivity = inactivityTime;
}

void
UeManager::ActivatePacketDuplication (uint16_t sourceCellId, uint16_t targetCellId, uint16_t targetRnti)
{
  NS_LOG_FUNCTION (this);
  m_packetDuplication = true;
  m_DCsourceCellId = sourceCellId;
  m_DCtargetCellId = targetCellId;
  m_DCtargetRnti = targetRnti;
}

void
UeManager::DeActivatePacketDuplication ()
{
  NS_LOG_FUNCTION (this);
  m_packetDuplication = false;
  m_DCsourceCellId = 0;
...
UeManager::SendRrcConnectionRelease ()
{
  ...
  if (!m_useInactiveState) { //Se mantiene contexto si usamos INACTIVE
      // Bearer de-activation indication towards epc-enb application
      std::map <uint8_t, Ptr<LteDataRadioBearerInfo> > all_bearers = m_drbMap;
    ...
    //TODO
    //Estado INACTIVO (esperar un ResumeRequest, una vez recibido activar bearer (InitialUE CONTEXT), enviar RRCResume y esperar RRCResumeCompleted para pasar a estado CONNECTED_NORMALLY)
    //INITIAL-UE creo que no debería enviarse, y tampoco eliminar las bearer en PGW-SGW-MME. Ver estándar y preguntar a Jorge
...
UeManager::SendData (uint8_t bid, Ptr<Packet> p)
{
  ...
  switch (m_state)
    ...
    case HANDOVER_PATH_SWITCH:
      {
        if (!m_useIdeal) {
          Simulator::Cancel(m_disconnectEventID);
          m_disconnectEventID = Simulator::Schedule(m_inactivity,&LteEnbRrc::RemoveUeByImsi,m_rrc,m_rnti);
        }
        NS_LOG_LOGIC ("queueing data on PDCP for transmission over the air");
        SendPacket (bid, p);

        if ((m_rrc->m_masterGnb == true) && (m_packetDuplication == true)){
          //Envia el paquete por la interfax X2 hacia el gNB destino
          Ipv4Header ipv4header, ipv4header2;
          UdpHeader udpHeader, udpHeader2;
          SeqTsHeader seqTs, seqTs2;

          LtePdcpHeader pdcpHeader;
          PdcpTag pdcpTag;
          uint32_t packetSize = p->GetSize(); //Aquí va incluido el header de PDCP y el de Ipv4Header (hay que copiar datos del paquete menos cabecera e insertarselos)
          ...
          //Se asume que se va a usar la app Client UDP, que inserta la cabecera seqTs. Si se usa otra app, habria que quitar esta cabecera.
          ipv4header2 = ipv4header;
          udpHeader2 = udpHeader;
          seqTs2 = seqTs;

          uint32_t serializedSize = pdcpHeader.GetSerializedSize() + ipv4header.GetSerializedSize() + udpHeader.GetSerializedSize() + seqTs.GetSerializedSize();
          Ptr<Packet> packet = Create<Packet> (packetSize-(serializedSize));
          packet->AddByteTag (pdcpTag, 1, pdcpHeader.GetSerializedSize ());
          packet->AddHeader (pdcpHeader); //Mismo header que el paquete original.
          packet->AddHeader (ipv4header2);
          packet->AddHeader (udpHeader2);
          packet->AddHeader (seqTs2);

          //A nivel PDCP llega bien el paquete, a nivel Aplicación en el UE no, pero como no hay tiempo de procesamiento
          //se realiza el análisis a nivel PDCP. EL paquete solo llega a la app si se envia por el master (por tema socket creo)
          NS_LOG_LOGIC ("transmitting data over X2 for packet duplication");
          EpcX2Sap::UeDataParams params;
          params.sourceCellId = m_DCsourceCellId;
          params.targetCellId = m_DCtargetCellId;
          params.gtpTeid = m_DCtargetRnti; //Al activar packet duplication se le pasa el target rnti a la función ActivatePacketDuplication
        ...
        if (m_rrc->m_admitRrcConnectionRequest == true)
          {
            ...
            if (!m_useIdeal) {
              NotifyConnectionRequest (m_imsi);
            }
...
void
UeManager::NotifyConnectionRequest(uint64_t m_imsi)
{
	NS_LOG_FUNCTION (this);
	m_rrc->m_s1SapProvider->NotifyConnectionRequest(m_imsi);
}
...
void
UeManager::RecvRrcResumeComplete (LteRrcSap::RrcResumeComplete msg)
{
  NS_LOG_FUNCTION (this);
  //Ver estándar por si hay que hacer algo aquí
}

void
UeManager::RecvRrcResumeRequest (LteRrcSap::RrcResumeRequest msg)
{
  NS_LOG_FUNCTION (this);
  switch (m_state)
    {
        ...
        if (m_rrc->m_s1SapProvider != 0)
          {
            ...
            LteRrcSap::RrcResume msg2;
            msg2.rrcTransactionIdentifier = GetNewRrcTransactionIdentifier ();
            m_rrc->m_rrcSapUser->SendRrcResume (m_rnti, msg2);
            SwitchToState (CONNECTED_NORMALLY);
            //SwitchToState (ATTACH_REQUEST);
          }
        else
...
UeManager::SwitchToState (State newState)
{
  ...
  switch (newState)
    ...
    case INACTIVE:
      m_pendingRrcConnectionReconfiguration = false; // Force Reconfiguration
      m_pendingStartDataRadioBearers = false;
      break;
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
   ...
   .AddAttribute ("InactivityTimer",
                  ...
                  MakeTimeAccessor (&LteEnbRrc::m_inactivity),
                  MakeTimeChecker ())
...
void
LteEnbRrc::SetConnectionMode (bool useIdeal)
{
  NS_LOG_FUNCTION (this);
  m_useIdeal = useIdeal;
}
...
void
LteEnbRrc::SetInactiveState (bool useInactiveState)
{
  NS_LOG_FUNCTION (this);
  m_useInactiveState = useInactiveState;
}
...
void
LteEnbRrc::SetMasterGnb (bool masterGnb)
{
  NS_LOG_FUNCTION (this);
  m_masterGnb = masterGnb;
}

void
LteEnbRrc::SetSecondaryGnb (bool secondaryGnb)
{
  NS_LOG_FUNCTION (this);
  m_secondaryGnb = secondaryGnb;
}

void
LteEnbRrc::ActivatePacketDuplication (uint16_t rnti, uint16_t sourceCellId, uint16_t targetCellId, uint16_t targetRnti)
{
  NS_LOG_FUNCTION (this);
  GetUeManager (rnti)->ActivatePacketDuplication (sourceCellId, targetCellId, targetRnti);
}

void
LteEnbRrc::DeActivatePacketDuplication (uint16_t rnti)
{
  NS_LOG_FUNCTION (this);
  GetUeManager (rnti)->DeActivatePacketDuplication ();
}
...
void
LteEnbRrc::DoRecvRrcResumeComplete (uint16_t rnti, LteRrcSap::RrcResumeComplete msg)
{
  NS_LOG_FUNCTION (this << rnti);
...
void
LteEnbRrc::DoRecvRrcResumeRequest (uint16_t rnti, LteRrcSap::RrcResumeRequest msg)
{
  NS_LOG_FUNCTION (this << rnti);
  GetUeManager (rnti)->RecvRrcResumeRequest (msg);
}
...
LteEnbRrc::DoRecvHandoverRequest (EpcX2SapUser::HandoverRequestParams req)
{
  ...
  ueManager->SetConnectionMode (m_useIdeal);
  ueManager->SetInactivityTimer (m_inactivity);
  ueManager->SetInactiveState (m_useInactiveState);
...
LteEnbRrc::DoRecvUeData (EpcX2SapUser::UeDataParams params)
{
  ...
  if (m_secondaryGnb == true) {
    GetUeManager (params.gtpTeid)->SendData (1, params.ueData); // (bearer, params.ueData)//TODO: ver que bid ponerle según el UE (por defecto bearer 1, que siempre va a estar presente)
  }
  else {
  ...
  else
    {
      if (it->second->GetState() == UeManager::CONNECTED_NORMALLY)
        { // disconnect only if the UE is connected normally
          ...
          if (!m_useInactiveState) { //Con el estado INACTIVO, no se elimina el contexto del UE
