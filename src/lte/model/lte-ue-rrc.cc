...
LteUeRrc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeRrc")
    ...
    .AddTraceSource ("ConnectionReconfiguration",
                     ...
                     "ns3::LteUeRrc::ImsiCidRntiTracedCallback")
    .AddTraceSource ("ConnectionReconfigurationPreamble",
                      "trace fired upon RRC connection reconfiguration",
                      MakeTraceSourceAccessor (&LteUeRrc::m_connectionReconfigurationPreambleTrace),
                      "ns3::LteUeRrc::ImsiCidRntiPreambleTracedCallback")
    .AddTraceSource ("ResumeReceivedPreamble",
                      "trace fired upon RRC connection resume",
                      MakeTraceSourceAccessor (&LteUeRrc::m_connectionResumePreambleTrace),
                      "ns3::LteUeRrc::ResumePreambleTracedCallback")
...
LteUeRrc::DoSendData (Ptr<Packet> packet, uint8_t bid)
{
  ...
  if (!m_useIdeal) {
    //dsr: Aquí tenemos que hacer una comprobación de si la conexión está ya establecida o no.
     ...
     {
       NS_LOG_WARN ("The connection was not established yet, lost packet");
     }
  }
  else {
    if (drbid != 0)
...
void
LteUeRrc::SetConnectionMode (bool useIdeal)
{
  NS_LOG_FUNCTION (this);
  m_useIdeal = useIdeal;
}
...
void
LteUeRrc::SetInactiveState (bool useInactiveState)
{
  NS_LOG_FUNCTION (this);
...
LteUeRrc::DoNotifyRandomAccessSuccessful ()
{
  ...
  switch (m_state)
    ...
    case INACTIVE:
      LteRrcSap::RrcResumeRequest msg2;
      msg2.resumeIdentity = m_imsi;
      msg2.resumeMac = m_rnti;
      m_rrcSapUser->SendRrcResumeRequest (msg2);
      break;
...
LteUeRrc::DoNotifyRandomAccessFailed ()
{
  ...
  switch (m_state)
    ...
    case INACTIVE:
      {
        m_asSapUser->NotifyConnectionFailed (); //Esto llama al nas y hace que vuelva a comenzar el proceso, se vuelve a enviar el preámbulo inmediatamente
      }
      break;
...
void
LteUeRrc::DoPowerOn () //TODO: Añadir esta función en lte-ue-rrc.h, lte-as-sap.cc y lte-as-sap.h
{
	NS_LOG_FUNCTION(this);
  NS_ASSERT_MSG(!m_useIdeal, "This function only works with non-ideal connections");
	switch (m_state)
	{
		case IDLE_START:
		case IDLE_CELL_SEARCH:
		case IDLE_WAIT_MIB_SIB1:
		case IDLE_WAIT_SIB1:
		case IDLE_WAIT_MIB: //dsr: Comentado. Si estamos esperando un MIB no tenemos por qué tener pendiente una conexión.
			/*m_connectionPending = true;
			 *     break;*/
			break;
		case IDLE_CAMPED_NORMALLY:
    ...
    case IDLE_CONNECTING:
      {
        if (m_useIdeal) {
          ...
          SwitchToState (CONNECTED_NORMALLY);
          m_cmacSapProvider.at (0)->NotifyConnectionSuccessful ();
          //Simulator::Schedule(MicroSeconds(1.0),&ns3::LteAsSapUser::NotifyConnectionSuccessful,m_asSapUser);
          m_connectionEstablishedTrace (m_imsi, m_cellId, m_rnti);
          NS_ABORT_MSG_IF (m_noOfSyncIndications > 0, "Sync indications should be zero "
                           "when a new RRC connection is established. Current value = " << (uint16_t) m_noOfSyncIndications);
        }
  ...
  switch (m_state)
    {
      case CONNECTED_NORMALLY:
      ...
      else
        {
          ...
          m_connectionReconfigurationPreambleTrace (m_imsi, m_cellId, m_rnti, m_TimeTxPreamble);
          if(!m_useIdeal){
            NS_LOG_INFO ("Reconfigurada conexión RRC, envío SwitchToState (ACTIVE) hacia epc-ue-nas.cc para enviar paquetes acumulados");
            //m_connectionReconfigurationPreambleTrace (m_imsi, m_cellId, m_rnti, m_TimeTxPreamble);
            m_asSapUser->SwitchToState (); //Para que envíe los datos almacenados en NAS una vez se tiene portadora dedicada.
          }
        }
      break;
...
void
LteUeRrc::DoRecvRrcResume (LteRrcSap::RrcResume msg)
{
  NS_LOG_FUNCTION (this);
  m_connectionResumePreambleTrace (m_imsi, m_cellId, m_rnti, m_TimeTxPreamble);
  // Enviar RESUME Complete al gNB y pasar a estado CONECTADO
  m_lastRrcTransactionIdentifier = msg.rrcTransactionIdentifier;
  LteRrcSap::RrcResumeComplete msg2;
  msg2.rrcTransactionIdentifier = m_lastRrcTransactionIdentifier;
  m_rrcSapUser->SendRrcResumeComplete (msg2);
  SwitchToState (CONNECTED_NORMALLY);
  //Esto de abajo se podría quitar y notificar directamente
  //Simulator::Schedule(MilliSeconds (20), &LteAsSapUser::NotifyConnectionSuccessful, m_asSapUser); //tiempo para que llegue RrcResumeComplete al eNB/gNB
  m_asSapUser->NotifyConnectionSuccessful ();
}
...
LteUeRrc::DoStartConnection ()
{
  ...
  if (m_useIdeal) {
    ...
    m_TimeTxPreamble = Simulator::Now();
    m_cmacSapProvider.at (0)->StartContentionBasedRandomAccessProcedure ();
  } else {
    if (m_useInactiveState && m_state == INACTIVE) {
      //Enviar RESUME Request al gNB. Luego recibir Resume Response y enviar RESUME COMPLETED y pasar a estado CONECTADO
      m_TimeTxPreamble = Simulator::Now();
      m_cmacSapProvider.at (0)->StartContentionBasedRandomAccessProcedure ();
        ...
        if(m_state != CONNECTED_NORMALLY) {
          if (m_hasReceivedSib2) {
            ...
            DoForceCampedOnEnb (m_previousCellId, m_dlEarfcn);
          }
      }
    }
  }
...
LteUeRrc::SwitchToState (State newState)
{
  ...
  switch (newState)
    ...
    case IDLE_WAIT_MIB_SIB1:
      if (!m_useIdeal) {
        NS_LOG_INFO(this << " Waiting for MIB and SIB1");
        break;
      }
    case IDLE_WAIT_MIB:
      if (!m_useIdeal) {
        NS_LOG_INFO(this << " Waiting for MIB");
        break;
      }
    case IDLE_WAIT_SIB1:
      if (!m_useIdeal) {
        NS_LOG_INFO(this << " Waiting for SIB1");
      }
      break;

    case IDLE_CAMPED_NORMALLY:
      if (m_useIdeal && m_connectionPending)
        {
          SwitchToState (IDLE_WAIT_SIB2);
        }
      break;
    ...
    case IDLE_WAIT_SIB2:
      NS_LOG_INFO(this << " Waiting for SIB2");
      if (m_useIdeal && m_hasReceivedSib2)
        {
          NS_ASSERT (m_connectionPending);
          StartConnection ();
        }
    ...
    case CONNECTED_NORMALLY:
    /* if (!m_useIdeal) {
       if (m_packetPending == true)
       {
         DoSendData (m_packet, m_bid);
       }
       break;
     } */
     break;
