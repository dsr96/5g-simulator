...
public:
 ...
 /**
   * \brief Tell the RRC entity to change from state IDLE_START to IDLE_CAMPED_NORMALLY.
   * 
   **/
  virtual void PowerOn() = 0;
  
  /**
   * \brief Starts the RRC and EPS bearer connection
   */
 ...
 *
 */
  virtual void ActivateEpsBearer () = 0;
...
template <class C>
void 
MemberLteAsSapProvider<C>::PowerOn ()
{
	m_owner->DoPowerOn ();
}
...
template <class C>
void 
MemberLteAsSapProvider<C>::StartConnection ()
{
	m_owner->DoStartConnection ();
}
...
public:
  ...
  virtual void Disconnect ();
  virtual void ActivateEpsBearer ();
...
template <class C>
void
MemberLteAsSapUser<C>::SwitchToState ()
{
  m_owner->DoSwitchToState ();
}
...
template <class C>
void
MemberLteAsSapUser<C>::ActivateEpsBearer ()
{
	m_owner->ActivateEpsBearer(EpsBearer::NGBR_VIDEO_TCP_PREMIUM,EpcTft::Default());
}
