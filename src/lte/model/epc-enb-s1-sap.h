...
public:
  ...
  /**
   * Inform the upper layer that a node is trying to connect.
   *
   * \param m_imsi
   */
  virtual void NotifyConnectionRequest(uint64_t m_imsi) = 0;
...
private:
  ...
  virtual void NotifyConnectionRequest(uint64_t m_imsi);
...
template <class C>
void MemberEpcEnbS1SapProvider<C>::NotifyConnectionRequest (uint64_t m_imsi)
{
	m_owner->DoNotifyConnectionRequest (m_imsi);
}
