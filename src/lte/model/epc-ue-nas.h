...
public:
  ...
  *
  * \param useInactiveState true if ideal
  */
 void SetInactiveState (bool useInactiveState);
  ...
  /**
   * \brief Causes NAS to tell AS to go to IDLE_CAMPED_NORMALLY state from
          IDLE_START state.
   */
  void PowerOn ();

  /**
   * \brief Causes NAS to tell AS to camp to a specific cell and go to IDLE_CAMPED_NORMALLY
   *        state.
   * \param cellId the id of the eNB to camp on
   * \param dlEarfcn the DL frequency of the eNB
   */
  void PowerOn (uint16_t cellId, uint32_t dlEarfcn);
