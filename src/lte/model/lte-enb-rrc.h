...
public:
   ...
   *
   * \param useIdeal true if ideal
   */
  void SetConnectionMode (bool useIdeal);

  /**
   * set 5G inactive state
   *
   * \param useInactiveState true if used
   */
  void SetInactiveState (bool useInactiveState);

  /**
   * Set the inactivity timeout
   *
   * \param inactivityTime maximum inactivity time
   */
  void SetInactivityTimer (Time inactivityTime);

  //Activa o desactiva la duplicación de paquetes para un UE específico.
  //Esto deberá gestionarse mediante un algoritmo y deberá decidirlo a priori el master gnb
  void ActivatePacketDuplication (uint16_t sourceCellId, uint16_t targetCellId, uint16_t targetRnti);

  void DeActivatePacketDuplication ();
  ...
  /**
   * Implement the LteEnbRrcSapProvider::RecvRrcResumeComplete interface.
   * \param msg RRC resume complete message
   */
  void RecvRrcResumeComplete (LteRrcSap::RrcResumeComplete msg);
  /**
   * Implement the LteEnbRrcSapProvider::RecvRrcResumeRequest interface.
   * \param msg RRC resume request message
   */
  void RecvRrcResumeRequest (LteRrcSap::RrcResumeRequest msg);
  /**
...
private:
  ...
  /**
   * Notifies the EpcEnbApplication of a new UE
  */
  void NotifyConnectionRequest(uint64_t m_imsi);
  ...
  /// Use ideal connection mode.
  bool m_useIdeal;

  /// Use 5G Inactive State
  bool m_useInactiveState;

  EventId m_disconnectEventID;
  Time m_inactivity;

  //Packet duplication via DualConnectivity
  bool m_packetDuplication;
  uint16_t m_DCsourceCellId;
  uint16_t m_DCtargetCellId;
  uint16_t m_DCtargetRnti;
...
public:
  ...
  /**
  * Select Master or secondary gnb for dual connectivity.
  * Por defecto false ambos
  */
  void SetMasterGnb (bool masterGnb);

  void SetSecondaryGnb (bool secondaryGnb);

  //Activa o desactiva la duplicación de paquetes para un UE específico.
  //Esto deberá gestionarse mediante un algoritmo y deberá decidirlo a priori el master gnb
  void ActivatePacketDuplication (uint16_t rnti, uint16_t sourceCellId, uint16_t targetCellId, uint16_t targetRnti);

  void DeActivatePacketDuplication (uint16_t rnti);
...
private:
  ...
  /**
   * Part of the RRC protocol. Forwarding LteEnbRrcSapProvider::RecvRrcCResumeComplete interface to UeManager::RecvRrcResumeComplete
   *
   * \param rnti the RNTI
   * \param msg the LteRrcSap::RrcResumeComplete
   */
  void DoRecvRrcResumeComplete (uint16_t rnti, LteRrcSap::RrcResumeComplete msg);
  /**
   * Part of the RRC protocol. Forwarding LteEnbRrcSapProvider::RecvRrcCResumeRequest interface to UeManager::RecvRrcResumeRequest
   *
   * \param rnti the RNTI
   * \param msg the LteRrcSap::RrcResumeRequest
   */
  void DoRecvRrcResumeRequest (uint16_t rnti, LteRrcSap::RrcResumeRequest msg);
  /**
  ...
  bool RemoveUeByImsi (uint16_t rnti);
