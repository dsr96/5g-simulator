...
void
LteUeRrcProtocolIdeal::DoSendRrcResumeComplete (LteRrcSap::RrcResumeComplete msg)
{
  // re-initialize the RNTI and get the EnbLteRrcSapProvider for the
  // eNB we are currently attached to
  m_rnti = m_rrc->GetRnti ();
  SetEnbRrcSapProvider ();
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
		       &LteEnbRrcSapProvider::RecvRrcResumeComplete,
                       m_enbRrcSapProvider,
		       m_rnti,
		       msg);
}
...
void
LteUeRrcProtocolIdeal::DoSendRrcResumeRequest (LteRrcSap::RrcResumeRequest msg)
{
  // re-initialize the RNTI and get the EnbLteRrcSapProvider for the
  // eNB we are currently attached to
  m_rnti = m_rrc->GetRnti ();
  SetEnbRrcSapProvider ();
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
		       &LteEnbRrcSapProvider::RecvRrcResumeRequest,
                       m_enbRrcSapProvider,
		       m_rnti,
		       msg);
}
...
void
LteEnbRrcProtocolIdeal::DoSendRrcResume (uint16_t rnti, LteRrcSap::RrcResume msg)
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
		       &LteUeRrcSapProvider::RecvRrcResume,
		       GetUeRrcSapProvider (rnti),
		       msg);
}
