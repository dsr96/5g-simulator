...
RrcResumeHeader::PreSerialize () const
{
  ...
  // Serialize DCCH message
  SerializeDlDcchMessage (6); //TODO MODIFICAR ESTE NUMERO PARA ESCOGER UNO QUE NO ESTE (Problablemente haya que aumentar numero de choice en RrcDlDcch)
                              //ya que antes utilizaba release 11 de LTE
  ...
  // Serialize RRCResume-IEs sequence:
  // 6 optional fields. Extension marker not present.
  std::bitset<6> options; //0 a todos, porque no están presentes los campos opcionales
  options.set (5,0); // No RadioBearerConfig
  options.set (4,0); // No masterCellGroup
  options.set (3,0); // No measconfig
  options.set (2,0); // No fullConfig
  options.set (1,0); // No lateNonCriticalExtension
  options.set (0,0); // No RRCResume-v1560-IEs
  SerializeSequence (options,false);
...
void
RrcResumeHeader::SetMessage (LteRrcSap::RrcResume msg)
{
  m_rrcTransactionIdentifier = msg.rrcTransactionIdentifier;
  m_isDataSerialized = false;
}
...
RrcResumeCompleteHeader::PreSerialize () const
{
  ...
  // Serialize DCCH message
  SerializeUlDcchMessage (6); //TODO MODIFICAR ESTE NUMERO PARA ESCOGER UNO QUE NO ESTÉ (Problablemente haya que aumentar numero de choice en RrcUlDcch)
                              //ya que antes utilizaba release 11 de LTE
  ...
  // Serialize RRCResumeComplete-IEs sequence:
  // 5 optional fields. Extension marker not present.
  std::bitset<5> options; //0 a todos, porque no están presentes los campos opcionales
  options.set (4,0); // No dedicatedNAS-Message
  options.set (3,0); // No selectedPLMN-Identity
  options.set (2,0); // No uplinkTxDirectCurrentList
  options.set (1,0); // No lateNonCriticalExtension
  options.set (0,0); // No nonCrititalExtension (RRCResumeComplete-v1610-IEs)
  SerializeSequence (options,false);
...
void
RrcResumeCompleteHeader::SetMessage (LteRrcSap::RrcResumeComplete msg)
{
  m_rrcTransactionIdentifier = msg.rrcTransactionIdentifier;
  m_isDataSerialized = false;
}

LteRrcSap::RrcResumeComplete
RrcResumeCompleteHeader::GetMessage () const
{
  LteRrcSap::RrcResumeComplete msg;

  msg.rrcTransactionIdentifier = m_rrcTransactionIdentifier;
  return msg;
}

//////////////////// RrcResumeRequest class ////////////////////////

RrcResumeRequestHeader::RrcResumeRequestHeader ()
{
  m_resumeIdentity = std::bitset<32> (0ul);
  m_resumeMac = std::bitset<16> (0ul);
  m_resumeCause = MO_DATA;
  m_spare = std::bitset<1> (0ul);
}
...
RrcResumeRequestHeader::PreSerialize () const
{
  ...
  // Serialize CCCH message
  SerializeUlCcchMessage (2); //TODO MODIFICAR ESTE NUMERO PARA ESCOGER UNO QUE NO ESTÉ (Problablemente haya que aumentar numero de choice en RrcUlCcch)
                              //ya que antes utilizaba release 11 de LTE
  ...
  // Serialize shortI-RNTI-Value ::= BIT STRING (SIZE (24))
  SerializeBitstring (m_resumeIdentity);

  // Serialize resumeMAC-I // BIT STRING (SIZE (16))
  SerializeBitstring (m_resumeMac);

  // Serialize ResumeCause : ResumeCause ::= ENUMERATED
  SerializeEnum (16,m_resumeCause);
...
RrcResumeRequestHeader::Deserialize (Buffer::Iterator bIterator)
{
  ...
  // Deserialize resumeMAC
  bIterator = DeserializeBitstring (&m_resumeMac,bIterator);

  // Deserialize establishmentCause
  bIterator = DeserializeEnum (16,&selectedOption,bIterator);
...
{
  os << "resumeIdentity: " << (((uint32_t) m_resumeIdentity.to_ulong ())) << std::endl;
  os << "resumeMAC: " << ((uint16_t) m_resumeMac.to_ulong()) << std::endl;
}

void
RrcResumeRequestHeader::SetMessage (LteRrcSap::RrcResumeRequest msg)
{
  m_resumeIdentity = std::bitset<32> (msg.resumeIdentity);
  m_resumeMac = std::bitset<16> (msg.resumeMac);
  m_isDataSerialized = false;
}

LteRrcSap::RrcResumeRequest
RrcResumeRequestHeader::GetMessage () const
{
  LteRrcSap::RrcResumeRequest msg;
  msg.resumeIdentity = ((uint32_t) m_resumeIdentity.to_ulong ());
  msg.resumeMac =   ((uint16_t) m_resumeMac.to_ulong());
