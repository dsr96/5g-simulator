...
public:
  ...
  virtual void NotifyConnectionRequest(uint64_t imsi) = 0;
...
private:
  MemberEpcS1apSapMme ();
  void NotifyConnectionRequest (uint64_t  imsi);
...
template <class C>
void MemberEpcS1apSapMme<C>::NotifyConnectionRequest (uint64_t imsi)
{
	m_owner->DoNotifyConnectionRequest (imsi);
}
