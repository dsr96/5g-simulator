...
private:
  ...
  /**
   * Part of the RRC protocol. Implement the LteUeRrcSapProvider::RecvRrcResume interface.
   * \param msg the LteRrcSap::RrcResume
   */
  void DoRecvRrcResume (LteRrcSap::RrcResume msg);
  ...
  Time m_TimeTxPreamble; //Tiempo en el cual se envía la orden para realizar la sincronización radio (preamble)
  ...
  /**
  * Indica la llegada de RRC reconfiguration complete (para no solapar la otra traza, aqui se añade un parámetro más, el tiempo en el cual se envió la orden de preambulo)
  *
   */
  TracedCallback<uint64_t, uint16_t, uint16_t, Time> m_connectionReconfigurationPreambleTrace;

  /**
  * Indica la llegada de RRC Resume ( se indica el tiempo en el cual se envió la orden de preambulo para calcular retardo)
  *
   */
  TracedCallback<uint64_t, uint16_t, uint16_t, Time> m_connectionResumePreambleTrace;
  ...
  ///True if there is any packet to be sent
  bool m_packetPending;
  ///bid stored for the next call of DoDataSend(Ptr<Packet> packet, uint8_t bid) when m_packetPending = true
  uint8_t m_bid;
  ///packet stored for the next call of DoDataSend(Ptr<Packet> packet, uint8_t bid) when m_packetPending = true
  Ptr<Packet> m_packet;
