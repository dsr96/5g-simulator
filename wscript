    ...
    try:
        for filename in os.listdir("scratch"):
            ...
            if os.path.isdir(os.path.join("scratch", filename)):
                ...
                obj.env.append_value("LIB", ["dlib"])
                obj.env.append_value('CCFLAGS', ['-std=c++11', '-03', '-lpthread', '-lX11', 'pkg-config --cflags --libs dlib-1']) #g++ -std=c++11 -O3 -lpthread -lX11 file_name.cpp `pkg-config --cflags --libs dlib-1`
    except OSError:
        return
