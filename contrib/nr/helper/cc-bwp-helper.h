...
struct BandwidthPartInfo
{
  ...
  enum Scenario
  {
    ...
    V2V_Urban, //!< V2V_Urban
    InF_SL, //!< Indoor Factory - sparse clutter, low BS - 38.901 v16.1.0
    InF_SL_LoS,
    InF_SL_nLoS,
    InF_DL, //!< Indoor Factory - dense clutter, low BS - 38.901 v16.1.0
    InF_DL_LoS,
    InF_DL_nLoS,
    InF_SH, //!< Indoor Factory - sparse clutter, hight BS - 38.901 v16.1.0
    InF_SH_LoS,
    InF_SH_nLoS,
    InF_DH, //!< Indoor Factory - dense clutter, hight BS - 38.901 v16.1.0
    InF_DH_LoS,
    InF_DH_nLoS
  } m_scenario {RMa};
...
public:
  ...
  struct SimpleOperationBandConf
  {
    ...
    uint8_t m_numBwp          {1};      //!< Number of BWP per CC
	  std::unordered_map<uint8_t, double> m_bwpBandwidthSlices; // DSR <BWPId, BWSlice>
   ...
   *
   */
  OperationBandInfo CreateOperationBandContiguousCcMod (const SimpleOperationBandConf &conf); //DSR - BWP with custom BW
...
private:
  ...
  void InitializeBwp (std::unique_ptr<BandwidthPartInfo> &bwp,
                      ...
                      uint8_t bwpId) const;
  void InitializeBwpMod (std::unique_ptr<BandwidthPartInfo> &bwp, //DSR - BWP with custom BW
                      double bwOfBwp, double lowerFreq, uint8_t bwpPosition,
                      uint8_t bwpId) const;
  ...
  std::unique_ptr<ComponentCarrierInfo> CreateCcMod (double ccBandwidth, double lowerFreq, //DSR - BWP with custom BW
                                                  uint8_t ccPosition, uint8_t ccId,
                                                  uint8_t bwpNumber, std::unordered_map<uint8_t, double>, BandwidthPartInfo::Scenario scenario);
  /**
