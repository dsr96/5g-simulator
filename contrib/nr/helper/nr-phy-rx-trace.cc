                                       ...
                                       uint64_t imsi, SpectrumValue& sinr, SpectrumValue& power)
{
  ...
  FILE* log_file;
  char fname[255];
  sprintf (fname, "UE_%llu_UL_SINR_dB-NR.txt", (long long unsigned ) imsi);
...
NrPhyRxTrace::ReportInterferenceTrace (uint64_t imsi, SpectrumValue& sinr)
{
  ...
  FILE* log_file;
  char fname[255];
  sprintf (fname, "UE_%llu_SINR_dB-NR.txt", (long long unsigned ) imsi);
  log_file = fopen (fname, "a");
  Values::iterator it = sinr.ValuesBegin ();
  while (it != sinr.ValuesEnd ())
    {
...
NrPhyRxTrace::ReportPowerTrace (uint64_t imsi, SpectrumValue& power)
{
  ...
  FILE* log_file;
  char fname[255];
  printf (fname, "UE_%llu_ReceivedPower_dB-NR.txt", (long long unsigned) imsi);
  log_file = fopen (fname, "a");
...
NrPhyRxTrace::ReportPacketCountUe (UePhyPacketCountParameter param)
{
  FILE* log_file;
  char fname[255];
  sprintf (fname,"UE_%llu_Packet_Trace-NR.txt", (long long unsigned) param.m_imsi);
  log_file = fopen (fname, "a");
  if (param.m_isTx)
    {
...
NrPhyRxTrace::ReportPacketCountEnb (GnbPhyPacketCountParameter param)
{
  FILE* log_file;
  char fname[255];
  sprintf (fname,"BS_%llu_Packet_Trace-NR.txt",(long long unsigned) param.m_cellId);
  log_file = fopen (fname, "a");
  if (param.m_isTx)
    {
      fprintf (log_file, "%d\t%d\t%d\n", param.m_subframeno, param.m_noBytes, 0);
...
NrPhyRxTrace::ReportDLTbSize (uint64_t imsi, uint64_t tbSize)
{
  FILE* log_file;
  char fname[255];
  sprintf (fname,"UE_%llu_Tb_Size-NR.txt", (long long unsigned) imsi);
  log_file = fopen (fname, "a");
...
NrPhyRxTrace::RxPacketTraceUeCallback (Ptr<NrPhyRxTrace> phyStats, std::string path, RxPacketTraceParams params)
{
  ...
  std::string node_id = path.substr(10,tam-10);
  std::size_t tam2 = path.find("/", tam+12);
  std::string device_id = path.substr(tam+12, tam2-(tam+12));
  if (!m_rxPacketTraceFile.is_open ())
    {
      std::ostringstream oss;
      ...
      m_rxPacketTraceFile << "Time" << "\t" << "nodeID" << "\t" <<
                             "deviceID" << "\t" << "direction" << "\t" <<
                             "frame" << "\t" << "subF" << "\t" << "slot" <<
                             "\t" << "1stSym" << "\t" << "nSymbol" <<
                             "\t" << "cellId" << "\t" << "rnti" <<
