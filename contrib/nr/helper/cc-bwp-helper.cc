...
void
CcBwpCreator::InitializeBwpMod (std::unique_ptr<BandwidthPartInfo> &bwp,
                             double bwOfBwp, double lowerFreq, uint8_t bwpPosition,
                             uint8_t bwpId) const
{
  NS_LOG_FUNCTION (this);
  bwp->m_centralFrequency = lowerFreq + bwOfBwp / 2;
  bwp->m_lowerFrequency = lowerFreq;
  bwp->m_higherFrequency = lowerFreq + bwOfBwp - 1;
  bwp->m_channelBandwidth = bwOfBwp;
  bwp->m_bwpId = bwpId;
  NS_LOG_INFO ("Initialize the " << +bwpPosition << "st (or nd) BWP of BW " <<
               bwOfBwp/1e6 << " MHz, from " << lowerFreq/1e6 <<
...
std::unique_ptr<ComponentCarrierInfo>
CcBwpCreator::CreateCcMod (double ccBandwidth, double lowerFreq, uint8_t ccPosition,
                        uint8_t ccId, uint8_t bwpNumber, std::unordered_map<uint8_t, double> bwpBandwidthSlices, BandwidthPartInfo::Scenario scenario)
{
  // Create a CC with a single BWP
  std::unique_ptr<ComponentCarrierInfo> cc (new ComponentCarrierInfo ());
  InitializeCc (cc, ccBandwidth, lowerFreq, ccPosition, ccId);
  ...
  //double bwpBandwidth = ccBandwidth / bwpNumber;
  double cc_lowerFrequency = cc->m_lowerFrequency;
  ...
  for (uint8_t i = 0; i < bwpNumber; ++i)
    {
      ...
      if (bwpNumber == 1)
      {
        InitializeBwp (bwp, ccBandwidth, cc->m_lowerFrequency, i, m_bandwidthPartCounter++);
      } else
      {
        InitializeBwpMod (bwp, bwpBandwidthSlices[i], cc_lowerFrequency, i, m_bandwidthPartCounter++);
        cc_lowerFrequency = cc_lowerFrequency + bwpBandwidthSlices[i];
      }
      bwp->m_scenario = scenario;
      bool ret = cc->AddBwp (std::move (bwp));
      NS_ASSERT (ret);
...
OperationBandInfo
CcBwpCreator::CreateOperationBandContiguousCcMod (const SimpleOperationBandConf &conf)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO ("Creating an op band formed by " << +conf.m_numCc << " contingous CC" <<
               " central freq " << conf.m_centralFrequency/1e6 << " MHz with BW " <<
  ...
  for (uint8_t ccPosition = 0; ccPosition < conf.m_numCc; ++ccPosition)
    {
      bool ret = band.AddCc (CreateCcMod (ccBandwidth, band.m_lowerFrequency,
                                       ccPosition, m_componentCarrierCounter++,
                                       conf.m_numBwp, conf.m_bwpBandwidthSlices, conf.m_scenario));
      NS_ASSERT (ret);
    }
