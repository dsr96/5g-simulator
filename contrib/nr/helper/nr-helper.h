...
public:
  ...
  /**
   * \brief Manual camping of the UEs in ueDevices on a given eNB.
   *
   * \param ueDevices UEs to be camped
   * \param enbDevices eNB on which the UEs are to be camped (lower distance)
   *
   * \warning This function can only be used in the EPC-enabled simulation
   * */
  void PowerOn (NetDeviceContainer ueDevices, NetDeviceContainer enbDevices);

  /**
   * \brief Manual camping of the UEs in ueDevices on a given eNB.
   *
   * \param ueDevice UE to be camped
   * \param enbDevices eNB on which the UEs are to be camped (lower distance)
   *
   * \warning This function can only be used in the EPC-enabled simulation
   * */
  void PowerOn (Ptr<NetDevice> ueDevice, NetDeviceContainer enbDevices);

  /**
   * \brief Manual camping of a UE device on a given eNB.
   *
   * \param ueDevice UE to be camped
   * \param enbDevice eNB on which the UE is to be camped
   *
   * \warning This function can only be used in the EPC-enabled simulation
   * */
  void PowerOn (const Ptr<NetDevice> &ueDevice, const Ptr<NetDevice> &gnbDevice);
  ...
  /**
   * \brief Enables the following traces for a specific node and device:
   * Transmitted/Received Control Messages
   * DL/UL Phy Traces
   * RLC traces
   * PDCP traces
   *
   */
  void EnableTracesForANodeDevice (std::string node, std::string device);
...
private:
  ...
  void EnableDlPhyTrace (std::string node, std::string device);
  void EnableUlPhyTrace (std::string node, std::string device);
  void EnableEnbPacketCountTrace (std::string node, std::string device);
  void EnableUePacketCountTrace (std::string node, std::string device);
  void EnableTransportBlockTrace (std::string node, std::string device);
  void EnableRlcTraces (std::string node, std::string device);
  void EnableGnbPhyCtrlMsgsTraces (std::string node, std::string device);
  void EnableUePhyCtrlMsgsTraces (std::string node, std::string device);
  void EnableGnbMacCtrlMsgsTraces (std::string node, std::string device);
  void EnableUeMacCtrlMsgsTraces (std::string node, std::string device);
  Ptr<NrBearerStatsCalculator> GetRlcStats (void);
  void EnablePdcpTraces (std::string node, std::string device);
  ...
  /**
   * The `UseIdealConnection` attribute. If true, ideal connections will be used
  */
  bool m_useIdealConnection;
  /**
  * The `UseInactiveState` attribute. If true, inactive state will be used
  */
