...
NrHelper::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute ("HarqEnabled",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("UseIdealConnection",
                 "If true, ideal connections will be used (uplink connections "
                  "start at the beginning of the simulation and never disconnect). "
               	 "If false, real connections are used (uplink connection start "
                 "when uplink data is sent and ends after a timeout. If UseIdealRrc "
                  "is true this will be ignored.",
                  BooleanValue (true),
                  MakeBooleanAccessor (&NrHelper::m_useIdealConnection),
                  MakeBooleanChecker ())
    .AddAttribute ("UseInactiveState",
                  "Use 5G Inactive STATE.",
                  BooleanValue (false),
                  MakeBooleanAccessor (&NrHelper::m_useInactiveState),
                  MakeBooleanChecker ())
    ;
  return tid;
}
...
static void
InitInFSL (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactorySLPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (ThreeGppIndoorFactorySLChannelConditionModel::GetTypeId ());
}

static void
InitInFSL_LoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactorySLPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (AlwaysLosChannelConditionModel::GetTypeId ());
}
...
static void
InitInFSL_nLoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactorySLPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (NeverLosChannelConditionModel::GetTypeId ());
}
...
static void
InitInFDL (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactoryDLPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (ThreeGppIndoorFactoryDLChannelConditionModel::GetTypeId ());
}

static void
InitInFDL_LoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactoryDLPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (AlwaysLosChannelConditionModel::GetTypeId ());
}
...
static void
InitInFDL_nLoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactoryDLPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (NeverLosChannelConditionModel::GetTypeId ());
}
...
static void
InitInFSH (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactorySHPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (ThreeGppIndoorFactorySHChannelConditionModel::GetTypeId ());
}

static void
InitInFSH_LoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactorySHPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (AlwaysLosChannelConditionModel::GetTypeId ());
}
...
static void
InitInFSH_nLoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactorySHPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (NeverLosChannelConditionModel::GetTypeId ());
}
...
static void
InitInFDH (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactoryDHPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (ThreeGppIndoorFactoryDHChannelConditionModel::GetTypeId ());
}

static void
InitInFDH_LoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactoryDHPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (AlwaysLosChannelConditionModel::GetTypeId ());
}
...
static void
InitInFDH_nLoS (ObjectFactory *pathlossModelFactory, ObjectFactory *channelConditionModelFactory)
{
  pathlossModelFactory->SetTypeId (ThreeGppIndoorFactoryDHPropagationLossModel::GetTypeId ());
  channelConditionModelFactory->SetTypeId (NeverLosChannelConditionModel::GetTypeId ());
}
...
NrHelper::InitializeOperationBand (OperationBandInfo *band, uint8_t flags)
{
  ...
  static std::unordered_map<BandwidthPartInfo::Scenario, InitPathLossFn, std::hash<int>> initLookupTable
  {
    ...
    {BandwidthPartInfo::InF_SL, std::bind (&InitInFSL, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_SL_LoS, std::bind (&InitInFSL_LoS, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_SL_nLoS, std::bind (&InitInFSL_nLoS, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_DL, std::bind (&InitInFDL, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_DL_LoS, std::bind (&InitInFDL_LoS, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_DL_nLoS, std::bind (&InitInFDL_nLoS, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_SH, std::bind (&InitInFSH, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_SH_LoS, std::bind (&InitInFSH_LoS, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_SH_nLoS, std::bind (&InitInFSH_nLoS, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_DH, std::bind (&InitInFDH, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_DH_LoS, std::bind (&InitInFDH_LoS, std::placeholders::_1, std::placeholders::_2)},
    {BandwidthPartInfo::InF_DH_nLoS, std::bind (&InitInFDH_nLoS, std::placeholders::_1, std::placeholders::_2)},
  };
  ...
  if (useIdealRrc)
    {
      ...
      rrc->SetConnectionMode (m_useIdealConnection);
      nas->SetConnectionMode (m_useIdealConnection);
      rrc->SetInactiveState(m_useInactiveState);
      nas->SetInactiveState(m_useInactiveState);
    }
  else
    {
...
/*
          TO DO Recordar que la conexión hay que hacerla en tres pasos:
                1) NrHelper::PowerOn
                2) NrHelper::AttachToEnb -modificado-
                3) NrHelper::ActivateDedicatedEpsBearer
*/

void
NrHelper::PowerOn (NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (!m_useIdealConnection, "This feature only works with non-ideal connections");

  for (NetDeviceContainer::Iterator i = ueDevices.Begin (); i != ueDevices.End (); i++)
    {
      PowerOn (*i, enbDevices);
    }
}

void
NrHelper::PowerOn (Ptr<NetDevice> ueDevice, NetDeviceContainer enbDevices)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (!m_useIdealConnection, "This feature only works with non-ideal connections");
  NS_ASSERT_MSG (enbDevices.GetN () > 0, "empty enb device container");
  ...
  PowerOn (ueDevice, closestEnbDevice);
}


void
NrHelper::PowerOn (const Ptr<NetDevice> &ueDevice,
                           const Ptr<NetDevice> &gnbDevice)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (!m_useIdealConnection, "This feature only works with non-ideal connections");
  ...
  for (uint32_t i = 0; i < enbNetDev->GetCcMapSize (); ++i)
    {
      ...
      Ptr<EpcUeNas> ueNas = ueNetDev->GetNas ();
      ueNas->PowerOn (enbNetDev->GetBwpId (i), enbNetDev->GetEarfcn (i));
    }
...
void
NrHelper::EnableTracesForANodeDevice (std::string node, std::string device)
{
  EnableDlPhyTrace (node,device);
  EnableUlPhyTrace (node,device);
  //EnableEnbPacketCountTrace (node,device);
  //EnableUePacketCountTrace (node,device);
  //EnableTransportBlockTrace (node,device);
  EnableRlcTraces (node,device);
  EnablePdcpTraces (node,device);
  //EnableGnbPhyCtrlMsgsTraces (node,device);
  //EnableUePhyCtrlMsgsTraces (node,device);
  //EnableGnbMacCtrlMsgsTraces (node,device);
  //EnableUeMacCtrlMsgsTraces (node,device);
}

void
NrHelper::EnableDlPhyTrace (std::string node, std::string device)
{
  //NS_LOG_FUNCTION_NOARGS ();
  //Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/ReportCurrentCellRsrpSinr",
                   //MakeBoundCallback (&NrPhyRxTrace::ReportCurrentCellRsrpSinrCallback, m_phyStats));

  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/SpectrumPhy/RxPacketTraceUe",
                   MakeBoundCallback (&NrPhyRxTrace::RxPacketTraceUeCallback, m_phyStats));
}

void
NrHelper::EnableGnbPhyCtrlMsgsTraces (std::string node, std::string device)
{
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/BandwidthPartMap/*/NrGnbPhy/GnbPhyRxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrPhyRxTrace::RxedGnbPhyCtrlMsgsCallback, m_phyStats));
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/BandwidthPartMap/*/NrGnbPhy/GnbPhyTxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrPhyRxTrace::TxedGnbPhyCtrlMsgsCallback, m_phyStats));
}

void
NrHelper::EnableGnbMacCtrlMsgsTraces (std::string node, std::string device)
{
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/BandwidthPartMap/*/NrGnbMac/GnbMacRxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrMacRxTrace::RxedGnbMacCtrlMsgsCallback, m_macStats));

  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/BandwidthPartMap/*/NrGnbMac/GnbMacTxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrMacRxTrace::TxedGnbMacCtrlMsgsCallback, m_macStats));
}

void
NrHelper::EnableUePhyCtrlMsgsTraces (std::string node, std::string device)
{
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/UePhyRxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrPhyRxTrace::RxedUePhyCtrlMsgsCallback, m_phyStats));
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/UePhyTxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrPhyRxTrace::TxedUePhyCtrlMsgsCallback, m_phyStats));
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/UePhyRxedDlDciTrace",
                   MakeBoundCallback (&NrPhyRxTrace::RxedUePhyDlDciCallback, m_phyStats));
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/UePhyTxedHarqFeedbackTrace",
                   MakeBoundCallback (&NrPhyRxTrace::TxedUePhyHarqFeedbackCallback, m_phyStats));
}

void
NrHelper::EnableUeMacCtrlMsgsTraces (std::string node, std::string device)
{
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUeMac/UeMacRxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrMacRxTrace::RxedUeMacCtrlMsgsCallback, m_macStats));
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUeMac/UeMacTxedCtrlMsgsTrace",
                   MakeBoundCallback (&NrMacRxTrace::TxedUeMacCtrlMsgsCallback, m_macStats));
}

void
NrHelper::EnableUlPhyTrace (std::string node, std::string device)
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/BandwidthPartMap/*/NrGnbPhy/SpectrumPhy/RxPacketTraceEnb",
                   MakeBoundCallback (&NrPhyRxTrace::RxPacketTraceEnbCallback, m_phyStats));
}

void
NrHelper::EnableEnbPacketCountTrace (std::string node, std::string device)
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/BandwidthPartMap/*/NrGnbPhy/SpectrumPhy/ReportEnbTxRxPacketCount",
                   MakeBoundCallback (&NrPhyRxTrace::ReportPacketCountEnbCallback, m_phyStats));

}

void
NrHelper::EnableUePacketCountTrace (std::string node, std::string device)
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/SpectrumPhy/ReportUeTxRxPacketCount",
                   MakeBoundCallback (&NrPhyRxTrace::ReportPacketCountUeCallback, m_phyStats));

}

void
NrHelper::EnableTransportBlockTrace (std::string node, std::string device)
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect ("/NodeList/"+node+"/DeviceList/"+device+"/ComponentCarrierMapUe/*/NrUePhy/ReportDownlinkTbSize",
                   MakeBoundCallback (&NrPhyRxTrace::ReportDownLinkTBSize, m_phyStats));
}


void
NrHelper::EnableRlcTraces (std::string node, std::string device)
{
  ...
  m_radioBearerStatsConnector.EnableRlcStats (m_rlcStats, node, device);
}
...
void
NrHelper::EnablePdcpTraces (std::string node, std::string device)
{
  ...
  m_radioBearerStatsConnector.EnablePdcpStats (m_pdcpStats, node, device);
}
