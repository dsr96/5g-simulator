...
public:
  ...
  virtual void SendRachPreamble (uint32_t PreambleId, uint16_t Rnti) override;
...
void
NrMemberPhySapProvider::SendRachPreamble (uint32_t PreambleId, uint16_t Rnti)
{
...
BeamId
NrMemberPhySapProvider::GetBeamId (uint16_t rnti) const
{
...
NrPhy::SendRachPreamble (uint32_t PreambleId, uint32_t Rnti)
{
  ...
  m_sendRaPreamble = true; //indica a la clase nr-ue-phy que se ha enviado un Ra, para cuando reciba RAR decodificarlo si ha sido el el que lo ha enviado
