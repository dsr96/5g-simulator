...
NrNetDevice::Receive (Ptr<Packet> p)
{
  ...
  if (p->PeekHeader (ipv4Header) != 0)
  ...
  else
    {
      ...
      //DSR: Cuando se utiliza PD puede llegar aquí y no poder obtener Ipv4 porque ha llegado por la otra interfaz antes
      // Se comparte la instancia del paquete, por eso el error.
      //Se obtiene si se ha recibido de las trazas de PDCP
      NS_LOG_INFO ("NrNetDevice::Receive - Unknown IP type...");
    }
}
