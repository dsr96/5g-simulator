...
NrUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUePhy")
    ...
    .AddTraceSource ("ReportCQIValue",
                     "Wideband CQI value [0-15]",
                     MakeTraceSourceAccessor (&NrUePhy::m_reportWbCQI),
                     "ns3::NrUePhy::WbCQIValueTracedCallback")
    .AddTraceSource ("ReportRSRPValue",
                     "RSRP value [dBm]",
                     MakeTraceSourceAccessor (&NrUePhy::m_reportRsrp),
                     "ns3::NrUePhy::RSRPValueTracedCallback")
      ;
  return tid;
}
...
NrUePhy::PhyCtrlMessagesReceived (const Ptr<NrControlMessage> &msg)
{
  ...
  if (msg->GetMessageType () == NrControlMessage::DL_DCI)
  ...
  else if (msg->GetMessageType () == NrControlMessage::RAR)
    {
      ...
      if (m_sendRaPreamble) { // si se ha enviado RACH, se decodifica y se pasa a la capa MAC
        Simulator::Schedule ((GetSlotPeriod () * (GetL1L2CtrlLatency ()/2)), &NrUePhy::DoReceiveRar, this, rarMsg);
        m_sendRaPreamble = false;
      }
    }
  else
    {
...
NrUePhy::CreateDlCqiFeedbackMessage (const SpectrumValue& sinr)
{
  ...
  //Añadir aqui traza para obtener el CQI
  //std::cout << "UE RNTI: " << dlcqi.m_rnti << "CQI value: " << unsigned(dlcqi.m_wbCqi) << std::endl;
  m_reportWbCQI (dlcqi.m_rnti, GetCellId(), dlcqi.m_wbCqi, m_amc->GetMcsFromCqi(dlcqi.m_wbCqi));
  msg->SetDlCqi (dlcqi);
  return msg;
}
      ...
      if (Simulator::Now () > m_wbCqiLast)
        {
          ...
          m_reportCurrentCellRsrpSinrTrace (GetCellId (), m_rnti, GetRsrp(), ComputeAvgSinr (sinr), GetBwpId ());
        }
    }
}
...
NrUePhy::ReportRsReceivedPower (const SpectrumValue& rsReceivedPower)
{
  ...
  m_reportRsrp (m_rsrp, GetCellId(), m_rnti);
...
NrUePhy::DoResetPhyAfterRlf ()
{
  ...
  DoReset (); //esto no hace nada, por que no está implementado, evitamos error para cuando usemos lte
}
